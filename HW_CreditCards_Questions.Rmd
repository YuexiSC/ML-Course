...
fn_dumb <- ((fn+tn)*(fn+fp)/(fn+tn+fp+tp)) %>% round 
fp_dumb <- ((fn+fp)*(fp+tp)/(fn+tn+fp+tp)) %>% round 
tn_dumb <- ((fn+tn)*(tn+tp)/(fn+tn+fp+tp)) %>% round 
tp_dumb <- ((tp+tn)*(fp+tp)/(fn+tn+fp+tp)) %>% round 
cft_dumb<-matrix(c(fn_dumb,fp_dumb,tn_dumb,tp_dumb),2) %>% print
...
```{r}
accuracy <- ((tp + fn)/(tp + tn + fp + fn) )%>% print
```
...
sprintf("recall     %f", recall <- (tp/(tp+tn)))
